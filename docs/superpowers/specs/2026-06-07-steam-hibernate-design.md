...
### 设计修正(2026-06-07):还原永远自动,不分模式

原"Manual / Auto 双模式"拆分不对。正确模型:**压缩的触发**可手动或自动;**还原永远是自动的** —— 用户点 Steam 的 Play 就自动唤醒,手动压的游戏也走同一套 ProjFS 自动唤醒,绝不要求"先开工具点 Restore 再玩"。这样体验才丝滑。纯手动 Restore 按钮降为"高级/兜底"操作(ProjFS 不可用时的回退)。

R1 就绪状态:目标机 Windows 11 build 26200,ProjFS 特性已启用、PrjFlt 服务运行中、无需重启。下一步是 ProjFS provider 最小 spike(验证占位能否让 Steam 显示 Play 且不重下)。

### (历史)两种运行模式(玩家可全局选,亦可按游戏覆盖)
